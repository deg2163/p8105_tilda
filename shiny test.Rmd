...
final = overall_1 %>% 
  ...
  select(ID, wave, SEX.x.x, CS006.x, physical_value, mental_value) %>% 
  mutate(
    physical = case_when(
    ...
    sex = case_when(
      SEX.x.x == "(1) Male" ~ "Male",
      SEX.x.x == "(2) Female" ~ "Female"),
    
    marital = case_when(
      CS006.x == "(1) Married" ~ "Married",
      CS006.x == "(2) Living with a partner as if married" ~ "Living w/ Partner",
      CS006.x == "(3) Single (never married)" ~ "Never married",
      CS006.x == "(4) Separated" ~ "Separated",
      CS006.x == "(5) Divorced" ~ "Divorced",
      CS006.x == "(6) Widowed" ~ "Widowed"),
  ...
  select(-physical_value, -mental_value, -SEX.x.x, -CS006.x)
...
plot_2 =  
  ...
  select(CS006, ID) %>% 
  merge(wave_3_data, by = "ID") %>% 
  select(ID, ICD10_01:ICD10_16, CS006, SEX) %>% 
  mutate(total_01 = sum(ICD10_01), total_02 = sum(ICD10_02), total_03 = sum(ICD10_03), 
